...
(function(){
  ...
  function renderFolios(){
    ...
    v.innerHTML='<div class="hs104-card"><div class="cc-toolbar"><div><strong>Folios</strong><div class="hs104-note">Puedes corregir o eliminar folios mientras no hayan sido utilizados.</div></div><button class="cc-btn cc-btn-primary" id="hs104Generate">Generar folios</button></div><div class="cc-inv-wrap"><table class="cc-ant-table"><thead><tr><th>FOLIO</th><th>ESTATUS</th><th>RESPONSABLE</th><th>PERSONA</th><th>CREADO</th><th>ACCIONES</th></tr></thead><tbody>'+((rows.map(x=>{const used=String(x.estatus||'').toUpperCase()==='UTILIZADO';return '<tr><td><strong>'+esc(x.folio)+'</strong></td><td>'+esc(x.estatus)+'</td><td>'+esc(x.responsable_nombre||'—')+'</td><td>'+esc(x.beneficiario_nombre||x.operador_nombre||'—')+'</td><td>'+fmt(x.created_at)+'</td><td><div class="hs104-actions">'+(used?'<span class="hs104-pill hs104-ok">Utilizado · protegido</span>':'<button type="button" class="cc-btn cc-btn-light" data-edit-folio="'+esc(x.id)+'"><i class="fa-solid fa-pen"></i> Editar</button><button type="button" class="cc-btn cc-btn-light" data-delete-folio="'+esc(x.id)+'" style="color:#b91c1c"><i class="fa-solid fa-trash"></i> Eliminar</button>')+'</div></td></tr>'}).join('')||'<tr><td colspan="6" style="text-align:center;padding:20px">Sin folios.</td></tr>')+'</tbody></table></div></div>';
  ...
  function editFolioFromList(x){
    ...
    if(st==='PENDIENTE_ACEPTACION'||st==='EN_CUSTODIA'){
      const a=(D.asignacionesResponsable||[]).find(a=>String(a.serie)===String(x.serie)&&String(a.anio)===String(x.anio)&&Number(x.consecutivo)>=Number(a.desde)&&Number(x.consecutivo)<=Number(a.hasta));
  ...
  function renderOperadores(){
    const v=view();if(!v)return;
    v.innerHTML='<div class="hs104-card"><div class="cc-toolbar"><div><strong>Folios entregados</strong><div class="hs104-note">Entrega hojas a Operadores o Beneficiarios y corrige asignaciones activas.</div></div><div class="hs104-actions"><select id="hs104OpType" class="cc-input"><option value="OPERADOR">Operadores</option><option value="BENEFICIARIO">Beneficiarios</option></select><input id="hs104OpSearch" class="cc-input" type="search" placeholder="Buscar persona..."><button class="cc-btn cc-btn-primary" id="hs104AssignOp">Asignar rango</button></div></div><div class="cc-inv-wrap"><table class="cc-ant-table"><thead><tr><th>FOLIO</th><th>PERSONA</th><th>RESPONSABLE</th><th>FECHA</th><th>ESTATUS</th><th>ACCIONES</th></tr></thead><tbody id="hs104OpBody"></tbody></table></div></div>';
  ...
  function openEditOperator(x){
    ...
    function openAssignPerson()  function openAssignPerson(){if(!perm('asignar_operador'))return alert('Sin permiso.');const aa=(D.asignacionesResponsable||[]).filter(x=>x.estatus==='ACEPTADA');if(!aa.length)return alert('No hay rangos aceptados por responsables.');const body='<form><div class="hs104-grid"><div class="cc-field"><label>Tipo *</label><select name="tipoPersona"><option value="OPERADOR">Operador</option><option value="BENEFICIARIO">Beneficiario</option></select></div><div class="cc-field"><label>Persona *</label><select name="personaId" required></select></div><div class="cc-field"><label>Custodia *</label><select name="asignacionId" required>'+options(aa,x=>x.responsableNombre+' · '+x.serie+'-'+x.anio+' · '+six(x.desde)+' a '+six(x.hasta))+'</select></div><div class="cc-field"><label>Desde *</label><input name="desde" type="number" min="0" max="999999" required></div><div class="cc-field"><label>Hasta *</label><input name="hasta" type="number" min="0" max="999999" required></div></div><div class="cc-field"><label>Observaciones</label><textarea name="observaciones"></textarea></div><div class="hs104-actions"><button type="button" class="cc-btn cc-btn-light" data-cancel>Cancelar</button><button type="submit" class="cc-btn cc-btn-primary">Asignar hojas</button></div></form>';
