...
`;
  ...
  async function openBeneficiary(){
    ...
    try{
      ...
      body.innerHTML='<form id="ccBen24Form"><div class="cc-grid"><div class="cc-field"><label>Cuenta *</label><select name="cuentaId" required>'+opts(accounts,x=>x.nombre)+'</select></div><div class="cc-field"><label>Beneficiario *</label><select name="responsableId" required>'+opts(bens,x=>x.nombre)+'</select></div><div class="cc-field"><label>Fecha *</label><input name="fecha" type="date" value="'+today()+'" required></div><div class="cc-field"><label>Método *</label><select name="metodoDepositoId" required>'+opts(methods,x=>x.nombre)+'</select></div><div class="cc-field"><label>Referencia</label><input name="referencia"></div></div><div class="cc-toolbar" style="margin-top:14px"><div><strong>Conceptos del anticipo</strong><div class="cc-note">Selecciona el concepto del catálogo y captura libremente el monto.</div></div><button type="button" class="cc-btn cc-btn-light" id="ccBen24Add">+ Agregar concepto</button></div><div id="ccBen24Rows" style="display:grid;gap:7px"></div><div style="display:flex;justify-content:flex-end;margin-top:8px"><div style="min-width:220px;text-align:right;padding:9px 12px;border-radius:10px;background:#eff6ff;border:1px solid #bfdbfe"><div style="font-size:10px;color:#475569;font-weight:800">TOTAL DEL ANTICIPO</div><div id="ccBen24Total" style="font-size:20px;font-weight:900;color:#1d4ed8">$0.00</div></div></div><div class="cc-field" style="margin-top:12px"><label>Observaciones</label><textarea name="observaciones"></textarea></div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px"><button type="button" class="cc-btn cc-btn-light" data-cancel>Cancelar</button><button type="submit" class="cc-btn cc-btn-primary">Crear anticipo</button></div></form>';
      const f=body.querySelector('#ccBen24Form'),box=body.querySelector('#ccBen24Rows'),totalEl=body.querySelector('#ccBen24Total');
      body.querySelector('[data-cancel]').onclick=()=>o.remove();
      const recalc=()=>{const total=[...box.querySelectorAll('[data-monto]')].reduce((s,x)=>s+Number(x.value||0),0);totalEl.textContent=total.toLocaleString('es-MX',{style:'currency',currency:'MXN'});return total};
      function addRow(){const used=new Set([...box.querySelectorAll('[data-concept]')].map(x=>x.value).filter(Boolean));const available=concepts.filter(x=>!used.has(x.id));if(!available.length)return alert('No hay más conceptos activos disponibles en el catálogo.');const row=document.createElement('div');row.className='cc-ant-ben-row';row.innerHTML='<div class="cc-field"><label>Concepto *</label><select data-concept required>'+opts(available,x=>x.nombre)+'</select></div><div class="cc-field"><label>Monto libre *</label><input data-monto type="number" min="0.01" step="0.01" placeholder="0.00" required></div><button type="button" class="cc-btn cc-btn-light" data-rm title="Quitar">×</button>';box.appendChild(row);row.querySelector('[data-rm]').onclick=()=>{row.remove();recalc()};row.querySelector('[data-monto]').addEventListener('input',recalc)}
