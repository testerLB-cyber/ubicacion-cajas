(function(){
  ...
  function render(tipo){
    ...
    const xs=rows(tipo);
    box.innerHTML=xs.length?'<div class="cc-inv-wrap"><table class="cc-ant-table"><thead><tr><th>NOMBRE</th><th>ESTATUS</th><th>ACCIÓN</th></tr></thead><tbody>'+xs.map(x=>'<tr><td><strong>'+esc(x.nombre)+'</strong></td><td>'+esc(x.estatus||'ACTIVO')+'</td><td><button class="cc-btn cc-btn-light" data-edit="'+esc(x.id)+'" data-tipo="'+tipo+'">Editar</button></td></tr>').join('')+'</tbody></table></div>':'<div class="cc-note" style="padding:16px">Sin registros. Agrega el primero.</div>';
  ...
  function openForm(tipo,id=''){
    ...
    const estatus=(x?.estatus||'ACTIVO');
    save(tipo,id,nombre,estatus);
  }
  async function save(tipo,id,nombre,estatus){
    try{
      const {data,error}=await sb().rpc('cc_hs_catalog_save',{p_tipo:tipo,p_id:id||null,p_nombre:nombre,p_estatus:estatus||'ACTIVO'});
  ...
  function install(){
    ...
    const b2=document.createElement('button');b2.type='button';b2.className='cc-config-nav-btn';b2.dataset.hsConfig='CLASIFICACION';b2.innerHTML='<i class="fa-solid fa-tags"></i><span>Clasificaciones</span><small>Catálogo para hojas de servicio</small>';b2.onclick=()=>section('CLASIFICACION');nav.appendChild(b2);
    const s1=document.createElement('div');s1.id='ccConfigHsTiposViaje';s1.className='cc-config-section';s1.style.display='none';s1.innerHTML='<div class="cc-config-card"><div class="cc-toolbar"><div><strong>Tipos de viaje</strong><div class="cc-note">Estos valores se mostrarán en la app móvil y en procesos de hojas de servicio.</div></div><button class="cc-btn cc-btn-primary" id="ccHsAddTipoViaje">Agregar</button></div><div id="ccHsTiposViajeList"></div></div>';panel.appendChild(s1);s1.querySelector('#ccHsAddTipoViaje').onclick=()=>openForm('TIPO_VIAJE');
    const s2=document.createElement('div');s2.id='ccConfigHsClasificaciones';s2.className='cc-config-section';s2.style.display='none';s2.innerHTML='<div class="cc-config-card"><div class="cc-toolbar"><div><strong>Clasificaciones</strong><div class="cc-note">Estas clasificaciones se mostrarán en la app móvil para cada hoja asignada.</div></div><button class="cc-btn cc-btn-primary" id="ccHsAddClasificacion">Agregar</button></div><div id="ccHsClasificacionesList"></div></div>';panel.appendChild(s2);s2.querySelector('#ccHsAddClasificacion').onclick=()=>openForm('CLASIFICACION');
  }
