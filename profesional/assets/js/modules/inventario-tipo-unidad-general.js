/* Tráfico App · Inventario · clasificación sistema + catálogo de tipos v8 */
(function(){
 'use strict';
 if(window.__CC_TIPO_UNIDAD_V8__)return;
 window.__CC_TIPO_UNIDAD_V8__=true;
 ...
 function options(current=''){
   return '<option value="">Selecciona tipo de unidad</option>'+tipos.filter(x=>x.estatus==='ACTIVO'||String(x.id)===String(current)).map(x=>'<option value="'+esc(x.id)+'" '+(String(x.id)===String(current)?'selected':'')+'>'+esc(x.nombre)+'</option>').join('');
 }
 function setSystemClass(base,tipo){
   if(!base||!tipo)return;
   const cat=String(tipo.categoria||'').toUpperCase()==='CARRO'?'CARRO':'CAJA';
   const opts=[...base.options];
   let opt=opts.find(o=>String(o.textContent||'').trim().toUpperCase()===cat);
   if(!opt&&cat==='CAJA')opt=opts.find(o=>String(o.textContent||'').trim().toUpperCase().includes('CAJA'));
 ...
 async function load(){
   ...
   }catch(e){console.warn('Tipos de unidad:',e)}finally{loading=false;}
 }
 function decorateModal(modal){
   ...
   let field=f.querySelector('[data-cc-tipo-unidad-catalogo]');
   if(!field){
     field=document.createElement('div');field.className='cc-field';field.dataset.ccTipoUnidadCatalogo='1';
     field.innerHTML='<label>Tipo de unidad</label><select name="tipoUnidadGeneralId" required></select><div style="font-size:9px;color:#64748b;margin-top:4px">Se toma del catálogo de Configuración. CAJA/CARRO se determina automáticamente.</div>';
   ...
   if(det.dataset.ccUnitKey!==key){det.dataset.ccUnitKey=key;det.dataset.ccInitialized='';}
   if(!det.dataset.ccInitialized){
     let current=String(mapped?.tipoUnidadGeneralId||'');
     if(!current){
       const sys=String(base.selectedOptions?.[0]?.textContent||'').trim().toUpperCase()==='CARRO'?'CARRO':'CAJA';
       current=String(tipos.find(x=>x.estatus==='ACTIVO'&&String(x.categoria||'').toUpperCase()===sys)?.id||'');
     }
     det.innerHTML=options(current);if(current)det.value=current;det.dataset.ccInitialized='1';
   }
   if(!det.dataset.ccBound){det.dataset.ccBound='1';det.addEventListener('change',()=>setSystemClass(base,tipoById(det.value)));}
   setSystemClass(base,tipoById(det.value));
 }
 ...
 document.addEventListener('submit',e=>{
   ...
   const typeId=String(det.value||''),tipo=tipoById(typeId);
   if(!typeId||!tipo){e.preventDefault();e.stopImmediatePropagation();alert('Selecciona el tipo de unidad.');return;}
   setSystemClass(base,tipo);
   const number=String(num.value||'').trim();let tries=0;
   const wait=()=>{if(!document.body.contains(modal)){persist(number,typeId).catch(err=>alert('La unidad se guardó, pero no se pudo sincronizar su tipo de unidad.\n\n'+err.message));return;}if(++tries<30)setTimeout(wait,250);};
   setTimeout(wait,300);
 },true);
 function decorateInventory(){
   ...
   [...body.querySelectorAll('tr')].forEach(tr=>{
     ...
     const nombre=String(x.tipoUnidadGeneralNombre||'SIN TIPO');
     const desired='<strong class="cc-unit-type-name" style="font-size:10px;color:#0f172a">'+esc(nombre)+'</strong>';
     if(td[1]&&td[1].innerHTML!==desired)td[1].innerHTML=desired;
   });
 }
 ...
 function openCatalogForm(x={}){
   ...
   ov.innerHTML='<div style="background:#fff;width:min(560px,96vw);border-radius:16px;overflow:hidden"><div style="background:#0f172a;color:#fff;padding:15px 18px;display:flex;justify-content:space-between"><strong>'+(x.id?'Editar':'Nuevo')+' tipo de unidad</strong><button type="button" data-x style="border:0;background:none;color:#fff;font-size:22px">×</button></div><form style="padding:18px"><div class="cc-field"><label>Nombre del tipo de unidad</label><input name="nombre" required value="'+esc(x.nombre||'')+'"></div><div class="cc-field" style="margin-top:10px"><label>Clasificación del sistema</label><select name="categoria"><option value="CAJA" '+(x.categoria==='CAJA'?'selected':'')+'>CAJA</option><option value="CARRO" '+(x.categoria==='CARRO'?'selected':'')+'>CARRO</option></select><div style="font-size:9px;color:#64748b;margin-top:4px">Esta clasificación controla los filtros Cajas/Carros del Inventario.</div></div><div class="cc-field" style="margin-top:10px"><label>Estatus</label><select name="estatus"><option '+(x.estatus!=='INACTIVO'?'selected':'')+'>ACTIVO</option><option '+(x.estatus==='INACTIVO'?'selected':'')+'>INACTIVO</option></select></div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:15px"><button type="button" class="cc-btn cc-btn-light" data-cancel>Cancelar</button><button class="cc-btn cc-btn-primary" type="submit">Guardar</button></div></form></div>';
 ...
 function installCatalog(){
   ...
   if(btn){btn.querySelector('span')&&(btn.querySelector('span').textContent='Tipos de unidad');btn.querySelector('small')&&(btn.querySelector('small').textContent='Catálogo general · clasificado CAJA/CARRO');}
