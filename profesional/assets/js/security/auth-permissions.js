...
window.ccSaveEditedUser=async function(userId){
 ...
 const beneficiarioHojas=!!document.getElementById('ccEditBeneficiarioHojas')?.checked;
 const bh=await sb.rpc('cc_admin_set_beneficiario_hojas',{p_user_id:userId,p_value:beneficiarioHojas});
 if(bh.error||!bh.data?.ok){alert(bh.error?.message||bh.data?.error||'Usuario guardado, pero no se pudo actualizar Beneficiario en Control de Hojas.');return;}
