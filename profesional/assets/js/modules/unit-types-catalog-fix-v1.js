/* Tráfico App · Catálogo maestro Tipos de unidad v2 · 2026-09-13 */
(function(){
  ...
  async function saveState(state,revision,op){
    ...
    for(let i=0;i<2;i++){
      ...
      if(data?.error==='CONFLICTO_DE_VERSION'&&i===0){ const fresh=await loadState(); state=fresh.state; revision=fresh.revision; continue; }
      throw new Error(data?.error||'No se pudo guardar');
    }
  }
  function modal(title,item,onSave){
    document.getElementById('ccUnitTypeFixModal')?.remove();
    const cat=String(item?.categoria||'CARRO').toUpperCase()==='CAJA'?'CAJA':'CARRO';
    ...
    ov.innerHTML='<div style="background:#fff;width:min(650px,96vw);border-radius:16px;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,.3)"><div style="background:#0f172a;color:#fff;padding:15px 18px;display:flex;justify-content:space-between"><strong>'+esc(title)+'</strong><button type="button" data-x style="border:0;background:none;color:#fff;font-size:22px">×</button></div><form style="padding:18px"><div class="cc-grid"><div class="cc-field"><label>Nombre del tipo *</label><input name="nombre" required value="'+esc(item?.nombre||'')+'" placeholder="Ej. ESTAQUITA, VAN, TORTON, CAJA SECA"></div><div class="cc-field"><label>Estatus</label><select name="estatus"><option value="ACTIVO" '+(item?.estatus!=='INACTIVO'?'selected':'')+'>ACTIVO</option><option value="INACTIVO" '+(item?.estatus==='INACTIVO'?'selected':'')+'>INACTIVO</option></select></div></div><div style="margin-top:14px;padding:14px;border:1px solid #dbeafe;background:#eff6ff;border-radius:12px"><div style="font-size:11px;font-weight:900;color:#1e3a8a;margin-bottom:8px">¿En qué listado debe mostrarse este tipo?</div><label style="display:flex;align-items:center;gap:9px;margin:7px 0;font-size:12px;font-weight:800;cursor:pointer"><input type="checkbox" name="esCaja" '+(cat==='CAJA'?'checked':'')+'> Mostrar como <b>CAJA</b></label><label style="display:flex;align-items:center;gap:9px;margin:7px 0;font-size:12px;font-weight:800;cursor:pointer"><input type="checkbox" name="esCarro" '+(cat==='CARRO'?'checked':'')+'> Mostrar como <b>CARRO</b></label><div class="cc-note" style="margin-top:8px">Marca solo una opción. Ejemplo: ESTAQUITA, VAN y TORTON normalmente irían como CARRO; CAJA SECA o REFRIGERADA como CAJA.</div></div><div class="cc-note" style="margin-top:10px">Este catálogo alimenta el campo <b>Tipo de unidad</b> al crear o editar unidades y controla automáticamente el listado Cajas/Carros.</div><div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px"><button type="button" class="cc-btn cc-btn-light" data-c>Cancelar</button><button type="submit" class="cc-btn cc-btn-primary">Guardar</button></div></form></div>';
    ...
    const caja=ov.querySelector('[name="esCaja"]'),carro=ov.querySelector('[name="esCarro"]');
    caja.onchange=()=>{ if(caja.checked) carro.checked=false; else if(!carro.checked) caja.checked=true; };
    carro.onchange=()=>{ if(carro.checked) caja.checked=false; else if(!caja.checked) carro.checked=true; };
  ...
  async function editType(item){
    modal(item?'Editar tipo de unidad':'Nuevo tipo de unidad',item||{},async fd=>{
      ...
      const categoria=fd.get('esCaja')==='on'?'CAJA':'CARRO';
      const obj={id:item?.id||('tipo_'+Date.now().toString(36)+Math.random().toString(36).slice(2,5)),nombre,categoria,estatus:String(fd.get('estatus')||'ACTIVO').toUpperCase()};
      ...
      // El catálogo manda: reclasificar las unidades ya existentes que usan este tipo.
      st.cajas=Array.isArray(st.cajas)?st.cajas:[];
      st.cajas=st.cajas.map(u=>String(u.tipoUnidadId)===String(obj.id)?{...u,tipoUnidadNombre:obj.nombre,categoriaUnidad:obj.categoria}:u);
      await saveState(st,loaded.revision,item?'EDITAR_TIPO_UNIDAD':'CREAR_TIPO_UNIDAD');
      if(typeof window.ccReloadFromDatabase==='function') await window.ccReloadFromDatabase();
      if(typeof window.ccRenderInventario==='function') window.ccRenderInventario();
      if(typeof window.ccRenderDashboard==='function') window.ccRenderDashboard();
    });
  }
  async function render(){
    ...
    try{
      ...
      box.innerHTML=list.length?'<div style="overflow:auto"><table class="cc-table"><thead><tr><th>Tipo de unidad</th><th>Se muestra como</th><th>Estatus</th><th></th></tr></thead><tbody>'+list.map(x=>'<tr><td><strong>'+esc(x.nombre||'')+'</strong></td><td><span class="cc-badge '+(String(x.categoria).toUpperCase()==='CAJA'?'cc-ok':'')+'">'+esc(String(x.categoria||'CARRO').toUpperCase()==='CAJA'?'CAJA':'CARRO')+'</span></td><td><span class="cc-badge '+(x.estatus==='INACTIVO'?'cc-off':'cc-ok')+'">'+esc(x.estatus||'ACTIVO')+'</span></td><td style="text-align:right"><button type="button" class="cc-btn cc-btn-light" data-ut-edit="'+esc(x.id)+'">Editar</button></td></tr>').join('')+'</tbody></table></div>':'<div class="cc-note">No hay tipos de unidad configurados.</div>';
  ...
  function ensureSection(){
    ...
    if(!sec){
      ...
      sec.innerHTML='<div class="cc-config-card"><div class="cc-config-section-head"><div><h4>🚚 Tipos de unidad</h4><p>Agrega ESTAQUITA, VAN, TORTON, CAJA, CARRO u otros tipos. Cada tipo se clasifica como Caja o Carro para controlar dónde aparece en Inventario.</p></div><button type="button" id="ccNuevoTipoUnidadFix" class="cc-btn cc-btn-primary"><i class="fa-solid fa-plus mr-1"></i>Agregar tipo</button></div><div id="ccTiposUnidadFixList" style="margin-top:14px"></div></div>';
