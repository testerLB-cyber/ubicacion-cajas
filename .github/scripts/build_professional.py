...
(function(){const root=window.TraficApp=window.TraficApp||{};root.modules=root.modules||{};root.modules.inventario={name:'inventario',open(){const btn=[...document.querySelectorAll('#controlCajasSection .cc-tab')].find(x=>(x.getAttribute('onclick')||'').includes("ccTab('inventario'"));if(btn&&typeof window.ccTab==='function')return window.ccTab('inventario',btn);},render(){if(typeof window.ccRenderInventario==='function')return window.ccRenderInventario();}};})();\n''',
    'modules/rentas.js': '''/* Tráfico App · Fachada modular segura · Rentas */
(function(){const root=window.TraficApp=window.TraficApp||{};root.modules=root.modules||{};root.modules.rentas={name:'rentas',open(){const btn=[...document.querySelectorAll('#controlCajasSection .cc-tab')].find(x=>(x.getAttribute('onclick')||'').includes("ccTab('renta'"));if(btn&&typeof window.ccTab==='function')return window.ccTab('renta',btn);},render(){if(typeof window.ccRenderRenta==='function')return window.ccRenderRenta();},history(){if(typeof window.ccRenderHistorial==='function')return window.ccRenderHistorial();}};})();\n''',
    'modules/mantenimiento-dot.js': '''/* Tráfico App · Fachada modular segura · Mantenimiento y DOT */
(function(){const root=window.TraficApp=window.TraficApp||{};root.modules=root.modules||{};root.modules.mantenimiento={name:'mantenimiento-dot',open(){const btn=[...document.querySelectorAll('#controlCajasSection .cc-tab')].find(x=>(x.getAttribute('onclick')||'').includes("ccTab('mantenimiento'"));if(btn&&typeof window.ccTab==='function')return window.ccTab('mantenimiento',btn);},render(){if(typeof window.ccRenderMantenimiento==='function')return window.ccRenderMantenimiento();}};})();\n''',
...
(function(){const root=window.TraficApp=window.TraficApp||{};root.modules=root.modules||{};root.modules.configuracion={name:'configuracion',open(){const btn=[...document.querySelectorAll('#controlCajasSection .cc-tab')].find(x=>(x.getAttribute('onclick')||'').includes("ccTab('configuracion'"));if(btn&&typeof window.ccTab==='function')return window.ccTab('configuracion',btn);},render(){if(typeof window.ccRenderConfiguracion==='function')return window.ccRenderConfiguracion();}};})();\n''',
}
...
def replace_script(match):
    ...
    target=js_dir/relative; target.parent.mkdir(parents=True,exist_ok=True); target.write_text(body.strip()+'\n',encoding='utf-8')
...
for relative,body in FACADE_MODULES.items():
    target=js_dir/relative; target.parent.mkdir(parents=True,exist_ok=True); target.write_text(body,encoding='utf-8')
...
readme=f'''# Tráfico App - Versión Profesional Paralela\n\nBase funcional: `app-correcta.html`.\n\n- Producción no se modifica.\n- `source-original.html` es copia exacta.\n- `index.html` conserva orden de ejecución original.\n- CSS extraídos: {css_count}.\n- JS originales organizados: {js_count}.\n- Fachadas de dominio: {len(FACADE_MODULES)}.\n\nBuild actual: `profesional-modular-v3`.\n'''
(OUT/'README.md').write_text(readme,encoding='utf-8')

if (OUT/'source-original.html').read_bytes()!=SRC.read_bytes(): raise SystemExit('El respaldo paralelo no coincide con app-correcta.html')
if marker not in (OUT/'index.html').read_text(encoding='utf-8'): raise SystemExit('No se generó la marca de build profesional v3')
if css_count==0 or js_count!=12: raise SystemExit(f'Extracción inesperada: CSS={css_count}, JS={js_count}')
