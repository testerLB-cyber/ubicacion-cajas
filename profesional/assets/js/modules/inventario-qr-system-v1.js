...
(function(){
 ...
 function showPreview(){
  ...
  const o=overlay('ccUnitQrPreviewModal',`<div style="width:min(1100px,98vw);max-height:95vh;background:#f1f5f9;border-radius:16px;overflow:hidden;display:flex;flex-direction:column"><div style="padding:14px 18px;background:#0f172a;color:#fff;display:flex;justify-content:space-between;align-items:center"><div><b>Vista previa · Imprimir QR</b><div style="font-size:10px;color:#cbd5e1">${previewItems.length} QR · ${pages.length} página(s) · 3×3 por hoja</div></div><button data-close style="border:0;background:none;color:#fff;font-size:22px">×</button></div><div style="padding:16px;overflow:auto;flex:1">${pages.map((pg,pi)=>`<div style="width:min(816px,100%);aspect-ratio:215.9/279.4;margin:0 auto 18px;background:#fff;padding:2.8%;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);gap:1.2%;box-shadow:0 5px 20px rgba(15,23,42,.15)">${pg.map(u=>`<div style="border:1px solid #d1d5db;border-radius:6px;padding:6px;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;overflow:hidden"><b style="font-size:13px">${esc(u.numero||'UNIDAD')}</b><div style="font-size:8px;color:#64748b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;text-align:center">${esc([u.tipo,u.descripcion].filter(Boolean).join(' · '))}</div><img src="${u.qr}" style="width:min(76%,165px);aspect-ratio:1;margin-top:5px"><div style="font-size:7px;margin-top:auto;color:#475569">Escanear para actualizar ubicación</div></div>`).join('')}${Array.from({length:9-pg.length},()=>'<div></div>').join('')}</div>`).join('')}</div><div style="padding:12px 16px;background:#fff;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center"><span style="font-size:10px;color:#64748b">La vista previa representa el acomodo final del PDF.</span><div><button id="ccUnitQrBack" class="cc-btn cc-btn-light" type="button">Volver</button> <button id="ccUnitQrExport" class="cc-btn cc-btn-primary" type="button"><i class="fa-solid fa-file-pdf mr-1"></i>Exportar PDF</button></div></div></div>`);
  o.querySelector('[data-close]').onclick=()=>o.remove();o.querySelector('#ccUnitQrBack').onclick=()=>{o.remove();openSelector();};o.querySelector('#ccUnitQrExport').onclick=exportPdf;
 }
 function exportPdf(){
