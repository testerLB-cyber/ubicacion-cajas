(function(){
  let ADMIN_DATA=null, ADMIN_TAB='PENDIENTES';
  ...
  function renderAdminRows(){
    ...
    if(ADMIN_TAB==='PENDIENTES'){
      ...
      const rows=ADMIN_DATA.historialHojas||[];root.innerHTML=rows.length?rows.map(x=>'<div class="admin-row"><h4>📄 '+escA(x.folio||'Hoja')+'</h4><small class="op">'+escA(x.operador||'—')+'</small><small>'+fmtDate(x.fecha)+' · '+escA(x.tipo||'')+'</small><small>'+escA(x.cliente||'—')+' · '+escA(x.tipoViaje||'—')+' · '+escA(x.clasificacion||'—')+'</small></div>').join(''):'<div class="admin-empty">Sin historial de hojas.</div>';
    }else{
      const rows=ADMIN_DATA.historialAnticipos||[];root.innerHTML=rows.length?rows.map(x=>'<div class="admin-row"><h4>💳 '+escA(x.folio||'Anticipo')+' · '+fmtMoney(x.monto)+'</h4><small class="op">'+escA(x.operador||'—')+'</small><small>'+fmtDate(x.fecha)+' · '+escA(x.concepto||'—')+'</small><small>'+escA(x.tipoDocumento||'—')+' · '+escA(x.estatus||'')+'</small></div>').join(''):'<div class="admin-empty">Sin historial de anticipos.</div>';
    }
  }
