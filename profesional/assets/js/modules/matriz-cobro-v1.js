...
(function(){
 ...
 async function load(){
  ...
  if(!sb()){body.innerHTML='<tr><td colspan="5" style="text-align:center;padding:24px">Conectando con Supabase…</td></tr>';return setTimeout(load,500)}
  body.innerHTML='<tr><td colspan="5" style="text-align:center;padding:24px">Cargando matriz…</td></tr>';
  try{const r=await sb().rpc('cc_matriz_cobro_list');if(r.error)throw r.error;D=r.data||D;const old=sel.value;sel.innerHTML='<option value="">Selecciona un cliente</option>'+D.clientes.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.nombre)+'</option>').join('');if(D.clientes.some(x=>String(x.id)===String(old)))sel.value=old;render()}catch(e){console.error(e);body.innerHTML='<tr><td colspan="5" style="padding:24px;text-align:center;color:#b91c1c">No se pudo cargar: '+esc(e.message||e)+'</td></tr>'}
 }
 function render(){
  ...
  if(!cid){body.innerHTML='<tr><td colspan="5" style="text-align:center;padding:28px;color:#64748b">Selecciona un cliente arriba para mostrar su matriz completa.</td></tr>';if(sum)sum.textContent='';return}
  let configured=0,na=0;
  body.innerHTML=D.servicios.map((s,i)=>{const p=price(cid,s),aplica=p?.aplica!==false;if(p&&aplica)configured++;if(p&&!aplica)na++;const suf=s.clasificacionManual?(String(s.tipoViaje||'').toUpperCase()==='RESGUARDO'?' / DÍA':' / HORA'):'';return '<tr data-row="'+i+'"><td><strong>'+esc(s.tipoViaje)+'</strong></td><td>'+esc(s.clasificacion||'Sin clasificación')+'</td><td><div style="display:flex;align-items:center;gap:6px"><span>$</span><input type="number" min="0" step="0.01" data-price value="'+(p&&aplica?Number(p.precio).toFixed(2):'')+'" placeholder="0.00" '+(!aplica?'disabled':'')+' style="width:145px">'+(suf?'<b style="font-size:9px;color:#64748b">'+suf+'</b>':'')+'</div></td><td style="text-align:center"><label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer;font-weight:800"><input type="checkbox" data-na '+(!aplica?'checked':'')+'> N/A</label></td><td><button type="button" class="cc-btn cc-btn-primary" data-save="'+i+'" '+(canEdit()?'':'disabled')+'>Guardar</button></td></tr>'}).join('');
 ...
 async function save(tr){
  ...
  try{const r=await sb().rpc('cc_matriz_cobro_save',{p_cliente_id:cid,p_tipo_viaje_id:s.tipoViajeId,p_clasificacion_id:s.clasificacionId||null,p_precio:precio,p_aplica:!na});if(r.error)throw r.error;b.textContent='Guardado ✓';await load()}catch(e){alert('No se pudo guardar.\n'+(e.message||e));b.disabled=false;b.textContent='Guardar'}
 }
