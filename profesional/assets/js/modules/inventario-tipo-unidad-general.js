/* Tráfico App · Inventario · tipos de unidad canónicos v4 */
(function(){
 'use strict';
 if(window.__CC_TIPO_UNIDAD_CANONICAL_V4__)return;
 window.__CC_TIPO_UNIDAD_CANONICAL_V4__=true;
 ...
 function ensureBaseCar(base,key){
   ...
   if(car){base.value=car.value;return true;}
   return false;
 }
 function decorateModal(modal){
   ...
   const sync=()=>{
     const k=String(num.value||'').trim().toUpperCase();
     ensureBaseCar(base,k);
     const cur=mapa.get(k)?.tipoUnidadGeneralId||mapa.get(k)?.tipoUnidadId||det.value||'';
     det.innerHTML=options(cur);
     const car=isCar(base)||!!mapa.get(k);
     field.style.display=car?'block':'none';det.required=car;
   };
 ...
 document.addEventListener('submit',e=>{
   ...
   const key=String(num.value||'').trim().toUpperCase();
   const car=isCar(base)||!!mapa.get(key);if(!car)return;
 ...
 function decorateInventory(){
   ...
   [...body.querySelectorAll('tr')].forEach(tr=>{const td=tr.children;if(td.length<3)return;const x=mapa.get(String(td[2]?.textContent||'').trim().toUpperCase());let tag=td[1]?.querySelector('.cc-car-type-detail');if(x?.tipoUnidadGeneralNombre||x?.tipoUnidadNombre){if(!tag){tag=document.createElement('div');tag.className='cc-car-type-detail';tag.style.cssText='font-size:9px;color:#1d4ed8;font-weight:900;margin-top:4px';td[1].appendChild(tag);}tag.textContent='Tipo unidad: '+(x.tipoUnidadGeneralNombre||x.tipoUnidadNombre);}else tag?.remove();});
 }
